...
export default function CardCourse({card, active, setActive, setEdit_course_flag, setSaved_course_id, setReload_course}) {
    const [modalActive, setModalActive] = useState(false);
    const [deleteCourse, setDeleteCourse] = useState(false);
    const [verDeleteCourse, setVerDeleteCourse] = useState();
    function DeleteCourseFunc(id) {
      setVerDeleteCourse(id);
    }
    useEffect(() => {
      if (verDeleteCourse) {
      const apiUrl = 'https://i-proffi.com/api/v1/courses/' + verDeleteCourse;
      setVerDeleteCourse(false);
      axios.delete(apiUrl).then((repos) => {
        console.log("hh");
        alert("Курс удален");
          ...
          <div className="catalog-card-desc-text">
            <div>
              ...
              <div className="cardOptions">
                <div onClick={() => {setSaved_course_id(card.course_id); setActive(true)}}>
              <img src={srcPen} alt="" className="penCard" />
              </div>
              <div onClick={() => {DeleteCourseFunc(card.course_id);}} style={{cursor: "pointer"}}>
